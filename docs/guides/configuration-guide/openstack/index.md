...
Each image has its own `*_image` parameter, while the `*_tag` parameters are usually derived from a
single service tag, `neutron_server_tag` from `neutron_tag` or `nova_api_tag` from `nova_tag` for
