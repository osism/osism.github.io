...
* Solution:
  ...
  * Once the root cause has been resolved fix loadbalancers stuck in state `PENDING_*`.  
    ...
    ```sh
    docker exec -it mariadb mariadb -uoctavia -p octavia
    ```
...
* Solution: [For an unknown reason Neutron OVN maintenance never converted existing OVN logical switches to include 'neutron:provnet-network-type' attribute in their `external_ids`](https://bugs.launchpad.net/neutron/+bug/2111498), therefore we need to explicitly sync the network type to OVN logical switches.
  ...
  ```console
  read -rs PW
  docker exec mariadb mariadb -uneutron -p${PW} neutron -N -B -e 'SELECT network_id, network_type FROM networksegments ORDER BY network_id;' | docker exec -i ovn_nb_db /usr/bin/tee /tmp/neutron-network-types
  ```
...
OSISM switched the key-value store from Redis to Valkey in OSISM 11. Pick the tab
that matches the release of the deployment.

<Tabs>
<TabItem value="redis" label="Redis (OSISM 10 and earlier)">
...
<TabItem value="valkey" label="Valkey (OSISM 11 and later)">
