...
- project waiting/implementation times
- unexpected errors
- major restructure work after the first rollout
- unexpected expenses

*Opensource benefits from the collaboration of its users and its developers.*

For this reason, we are collecting questions, important topics to be clarified and hints to make it easier for users of this technology to be successful with it.
We would be very pleased if specific experiences from users were contributed to this list.
...
The installation can be carried out via IPv4 or IPv6 as well as hybrid.
  ...
  * Frontend Access: A dedicated IP adress space / network for services published by the cloud platform and its users
     * this is in most cases a public IPv4 network
     * at least TCP port 443 should be accessible for all adresses of this network from other networks
