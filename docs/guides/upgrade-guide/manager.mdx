...
adjustments are necessary. Read the release notes even if you are only updating from e.g. 7.0.2 to 7.1.2.

:::

The update of a manager service with a stable release of OSISM is described here.
In the example, OSISM release 7.1.2 is used.
...
2. Login to the manager using the "dragon" user
   ```
   ssh dragon@<MANAGER_NODE>
   ```

3. Update the configuration repository on the manager node.
...
4. Update the manager service on the manager node.
...
7. If Traefik is used on the manager node (`traefik_enable: true` in `environments/infrastructure/configuration.yml`)
...
8. Optional: Tag the upgrade in Git
   ```
   git tag "$(grep "manager_version:" environments/manager/configuration.yml|awk '{print $2}')"
   git push --tags
   ```
