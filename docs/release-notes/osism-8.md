...
Without the parameter, all OpenStack services receive `rabbit_quorum_queue = true` in the
  `[oslo_messaging_rabbit]` section of their configuration file and declare their RPC topic
  queues and notification queues as quorum queues instead of classic queues. Transient queues
  (RPC reply and fanout queues) remain classic queues, since `rabbit_transient_quorum_queue`
  stays disabled.
